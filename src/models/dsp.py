...
from engine.exceptions import AudioProcessingError, FileIOError
...
class AudioEngine:
    ...
    def _build_mastering_chain(self) -> Pedalboard:
        ...
        plugins.extend(
            [
                Compressor(
                    threshold_db=-14.0, ratio=3.0, attack_ms=5.0, release_ms=50.0
                ),
    ...
    def process_file(self, input_path: Path, output_path: Path) -> Iterator[int]:
        ...
        try:
            with AudioFile(str(input_path)) as f_in:
                ...
                with AudioFile(
                    str(output_path), "w", samplerate=samplerate, num_channels=channels
                ) as f_out:
            ...
            raise AudioProcessingError(
                f"Failed to process {input_path.name}: {str(e)}"
            ) from e

    def reset_engine(self) -> None:
        """Resets the underlying pedalboard state."""
        self.board = self._build_mastering_chain()
